    ...
    def constraintsHardware(self, componentsRequirements):
        ...
        for k in range(self.nrVM):
            for i in range(self.nrComp):
                self.solver.add(Implies(Not(self.a[i * self.nrVM + k]), self.PriceProv[k] == 0))
        ...
        for k in range(self.nrVM):
            ...
            sLst = []
            for i in range(self.nrComp):
                self.solver.add(Implies(Not(self.a[i * self.nrVM + k]), 0 == self.p[i * self.nrVM + k]))
                self.solver.add(Implies(Not(self.a[i * self.nrVM + k]), 0 == self.m[i * self.nrVM + k]))
                self.solver.add(Implies(Not(self.a[i * self.nrVM + k]), 0 == self.s[i * self.nrVM + k]))

                self.solver.add(Implies(self.a[i * self.nrVM + k], self.p[i * self.nrVM + k] == componentsRequirements[i][0]))
                self.solver.add(Implies(self.a[i * self.nrVM + k], self.m[i * self.nrVM + k] == componentsRequirements[i][1]))
                self.solver.add(Implies(self.a[i * self.nrVM + k], self.s[i * self.nrVM + k] == componentsRequirements[i][2]))
