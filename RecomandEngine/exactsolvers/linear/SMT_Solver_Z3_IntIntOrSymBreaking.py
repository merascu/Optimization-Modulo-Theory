...
class Z3_Solver(ManeuverProblem):
    ...
    def __defineVariablesAndConstraints(self):
        ...
        for j in range(self.nrVM):
           ...
           self.solver.add(Implies(sum([self.a[i+j] for i in range(0, len(self.a), self.nrVM)]) == 0,
                                   self.PriceProv[j] == 0))
            ...
            for conflictCompId in conflictCompsIdList:
                ...
                if self.solverTypeOptimize:
                    ...
                    self.solver.assert_and_track(
                        sum([self.a[alphaCompId * self.nrVM + j],
                             self.a[conflictCompId * self.nrVM + j]]) <= 1
    ...
    def RestrictionUpperLowerEqualBound(self, compsIdList, bound, operator):
        ...
        if operator == "<=":
            if self.solverTypeOptimize:
                ...
                self.__constMap[str("LabelUpperLowerEqualBound" + str(self.labelIdx))] = sum([self.a[compId * self.nrVM + j]
                                                                            for compId in compsIdList for j in range(self.nrVM)]) <= bound
                self.solver.assert_and_track(sum([self.a[compId * self.nrVM + j] for compId in compsIdList for j in range(self.nrVM)])
                    <= bound, "LabelUpperLowerEqualBound" + str(self.labelIdx))
                self.labelIdx += 1
        elif operator == ">=":
            if self.solverTypeOptimize:
                self.solver.add(
                    sum([self.a[compId * self.nrVM + j] for compId in compsIdList
                         for j in range(self.nrVM)]) >= bound)
            else:
    ...
    def RestrictionFullDeployment(self, alphaCompId, notInConflictCompsIdList):
        ...
        for j in range(self.nrVM):
            if self.solverTypeOptimize:
                self.solver.add(
                    (sum([self.a[alphaCompId * self.nrVM + j]]+[self.a[_compId * self.nrVM + j]
    ...
    def run(self, smt2lib, smt2libsol):
        ...
        if not self.solverTypeOptimize:
            ...
            for cc in c:
                #print("cc=", self.__constMap[cc])
                self.problem.logger.debug("Constraint label: {} "
                                          "constraint description {}".format(str(cc), self.__constMap[cc]))
