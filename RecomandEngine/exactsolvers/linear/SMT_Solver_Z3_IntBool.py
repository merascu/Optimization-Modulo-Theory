...
class Z3_Solver(ManeuverProblem):
    ...
    def __defineVariablesAndConstraints(self):
        ...
        self.p = [Real("p%i_%i" % (k + 1, i + 1)) for k in range(self.nrComp) for i in range(self.nrVM)]
        self.m = [Real("m%i_%i" % (k + 1, i + 1)) for k in range(self.nrComp) for i in range(self.nrVM)]
        self.s = [Real("s%i_%i" % (k + 1, i + 1)) for k in range(self.nrComp) for i in range(self.nrVM)]
    ...
    def RestrictionManyToManyDependency(self, alphaCompId, betaCompId, relation):
        ...
        if relation == "<=":
            if self.solverTypeOptimize:
                ...
                bvarsl = [self.a[alphaCompId * self.nrVM + j] for j in range(self.nrVM)]
                bvarsr = [self.a[betaCompId * self.nrVM + j] for j in range(self.nrVM)]
                self.solver.assert_and_track(PbLe([(x, 1) for x in bvarsl] + [(x, -1) for x in bvarsr], 0), "LabelManyToMany1: " + str(self.labelIdx))
                self.labelIdx += 1
        elif relation == ">=":
            if self.solverTypeOptimize:
                bvarsl = [self.a[alphaCompId * self.nrVM + j] for j in range(self.nrVM)]
                ...
                bvarsl = [self.a[alphaCompId * self.nrVM + j] for j in range(self.nrVM)]
                bvarsr = [self.a[betaCompId * self.nrVM + j] for j in range(self.nrVM)]
                self.solver.assert_and_track(PbGe([(x, 1) for x in bvarsl], [(x, 1) for x in bvarsr]),
            ...
            if self.solverTypeOptimize:
                ...
                bvarsl = [self.a[alphaCompId * self.nrVM + j] for j in range(self.nrVM)]
                bvarsr = [self.a[betaCompId * self.nrVM + j] for j in range(self.nrVM)]
                self.solver.assert_and_track(PbEq([(x, 1) for x in bvarsl], [(x, 1) for x in bvarsr]),
    ...
    def RestrictionOneToManyDependency(self, alphaCompId, betaCompId, noInstances):
        ...
        if self.solverTypeOptimize:
            ...
            bvars2 = [(self.a[betaCompId * self.nrVM + j], -1) for j in range(self.nrVM)]
            bvars = bvars1 + bvars2
            self.solver.assert_and_track(PbGe(bvars, 0), "LabelOneToMany: " + str(self.labelIdx))
            self.labelIdx += 1
        ...
        if self.solverTypeOptimize:
            ...
            bvars2 = [(self.a[betaCompId * self.nrVM + j], -1) for j in range(self.nrVM)]
            bvars = bvars1 + bvars2
            self.solver.assert_and_track(PbLe(bvars, 1 + noInstances))
            self.labelIdx += 1
    ...
    def RestrictionRangeBound(self, compsIdList, lowerBound, upperBound):
        ...
        if self.solverTypeOptimize:
            bvars = [self.a[compId * self.nrVM + j] for compId in compsIdList for j in range(self.nrVM)]
            self.solver.add(PbGe([(x, 1) for x in bvars], lowerBound))
        else:
            bvars = [self.a[compId * self.nrVM + j] for compId in compsIdList for j in range(self.nrVM)]
            self.solver.assert_and_track(PbGe([(x, 1) for x in bvars], lowerBound), "LabelRangeBound: " + str(self.labelIdx))
            self.labelIdx += 1
        if self.solverTypeOptimize:
            bvars = [self.a[compId * self.nrVM + j] for compId in compsIdList for j in range(self.nrVM)]
            self.solver.add(PbLe([(x, 1) for x in bvars], upperBound))
        else:
            self.solver.assert_and_track(PbLe([(x, 1) for x in bvars], upperBound), "LabelRangeBound: " + str(self.labelIdx))
            self.labelIdx += 1
    ...
    def RestrictionFullDeployment(self, alphaCompId, notInConflictCompsIdList):
        ...
        for j in range(self.nrVM):
            if self.solverTypeOptimize:
                ...
                bvars3 = [self.a[i + j] for i in range(0, len(self.a), self.nrVM)]
                bvars = bvars1 + bvars2
                self.solver.assert_and_track(Implies(PbGe([(x, 1) for x in bvars3], 1), PbEq([(x, 1) for x in bvars], 1)),
    ...
    def RestrictionRequireProvideDependency(self, alphaCompId, betaCompId, alphaCompIdInstances, betaCompIdInstances):
        ...
        if self.solverTypeOptimize:
            ...
            bvars2 = [(self.a[betaCompId * self.nrVM + j], -betaCompIdInstances) for j in range(self.nrVM)]
            bvars = bvars1 + bvars2
            self.__constMap["LabelRequireProvide: " + str(self.labelIdx)] = PbLe( [x for x in bvars], 0)
            self.solver.assert_and_track(PbLe( [x for x in bvars], 0), "LabelRequireProvide: " + str(self.labelIdx))
            self.labelIdx += 1
    ...
    def RestrictionAlphaOrBeta(self, alphaCompId, betaCompId):
        ...
        if self.solverTypeOptimize:
            ...
            bvars1 = [self.a[betaCompId * self.nrVM + j] for j in range(self.nrVM)]
            bvars2 = [self.a[betaCompId * self.nrVM + j] for j in range(self.nrVM)]
            self.solver.assert_and_track(Or(PbEq([(x, 1) for x in bvars1], 0),
                               PbGe([(x, 1) for x in bvars2], 1)),
        ...
        if self.solverTypeOptimize:
            ...
            bvars2 = [self.a[alphaCompId * self.nrVM + j] for j in range(self.nrVM)]
            bvars = bvars1 + bvars2
            self.solver.assert_and_track(PbGe([(x, 1) for x in bvars], 1), "LabelAlphaOrBeta: " + str(self.labelIdx))
            self.labelIdx += 1
