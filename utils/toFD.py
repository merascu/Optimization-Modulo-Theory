        ...
        for name in filteredFilesLst:
            ...
            with open(inputFile, "rt") as inp:
                with  open(outputFile, "w+") as out:
                    for line in inp:
                        cpu = re.match(r"\(declare\-fun\s+p\d_\d\s+\(\)\s+Int\)", line)
                        mem = re.match(r"\(declare\-fun\s+m\d_\d\s+\(\)\s+Int\)", line)
                        sto = re.match(r"\(declare\-fun\s+s\d_\d\s+\(\)\s+Int\)", line)
                        # print("cpu ", cpu)
                        # print("mem ", mem)
                        # print("sto ", sto)
                        if cpu != None:
                            cmCPU = re.search(r"\d_\d", cpu.group(0)).group(0).split("_")
                            ...
                            cmMem = re.search(r"\d_\d", mem.group(0)).group(0).split("_")
                            ...
                            cmSto = re.search(r"\d_\d", sto.group(0)).group(0).split("_")
...
def main():
    toFiniteDomain("../experimentalResults/linear/output_SMT_Solver_Z3_RealPBC_assertsoft/SMT2",
                   "../experimentalResults/linear/output_SMT_Solver_Z3_FD/SMT2",
